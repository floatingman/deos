...
I had a bit of trouble with history in eshell and it seems the answer was to disable history then
start emacs and go into an eshell. Quit emacs and then enable history and all should be well. So if
you get a weird error when trying to run commands in eshell, this may be the only way to fix it.
